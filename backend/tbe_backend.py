...
def clean_channel(value):
    """Normalizes channel IDs (e.g., 'CH-03' -> 'CH-03') for cross-sheet matching."""
    if pd.isna(value):
        return ""
...
# =========================================================
# MAIN PROCESSING CORE LOGIC (TBE SPECIFIC)
# =========================================================
def process_tbe_dashboard_data():
    ...
    IS_UPDATING = True
    print(f"[{datetime.now()}] REFRESHING TBE DASHBOARD ENGINE...")
    ...
    try:
        ...
        # ---------------------------------------------------------
        # RULE 1: TARGET QTY LOOKUP MAP (From MO_Data Sheet)
        # ---------------------------------------------------------
        target_qty_lookup = {}
        ...
        # ---------------------------------------------------------
        # RULE 2: ANCHOR BASE LINES FROM CHANNEL DATA (TRB & DGBB)
        # ---------------------------------------------------------
            ...
            for _, row in df.iterrows():
                ...
                if agg_key not in tbe_aggregation:
                    tbe_aggregation[agg_key] = {
                        "mo_number": raw_mo,
                        "mo_group": mo_group,
                        "product_variant": family_item,
                        "target_qty": target_qty_lookup.get(mo_group, 0),
                        "ring_type": ring_type,
                        "channel_id": channel_id, # Storing Channel for Transit Buffer Linkage
                        "sho_qty": 0.0,
                        "sho_in_date": "-", 
                        "tb_qty": 0.0,
                        "tb_out_date": None,
        ...
        # ---------------------------------------------------------
        # RULE 3: NEW RING WT TRANSIT BUFFER CALCULATIONS (Ch# & TYPE Matching)
        # ---------------------------------------------------------
        for _, df in transit_buffer_sheets.items():
            # Exact columns from the user's provided sample data
            ...
            for _, row in df.iterrows():
                ...
                # The primary quantity metric is "No Of Rings" 
                qty_tb = clean_nan(row.get(qty_col))
                tb_date = parse_date_safe(row.get(date_col))

                # Link back to main aggregation using Channel Number & Ring Type
                for agg_key, meta in tbe_aggregation.items():
                    if meta["channel_id"] == tb_channel and meta["ring_type"] == row_ring_type:
                        meta["tb_qty"] += qty_tb
                        
                        if tb_date:
                            if meta["tb_out_date"] is None or tb_date > meta["tb_out_date"]:
                                meta["tb_out_date"] = tb_date

        # ---------------------------------------------------------
        # COMPILING DATAFRAME GENERATION FOR FRONTEND BINDING
        # ---------------------------------------------------------
        compiled_summary = []
        for agg_key, meta in tbe_aggregation.items():
            
            if meta["max_cumulative"] == 0 and meta["sho_qty"] == 0:
                tracking_status = "Yet to Start"
            elif meta["max_cumulative"] >= meta["sho_qty"] and meta["sho_qty"] > 0:
        ...
        print(f"[{datetime.now()}] TBE DATA ENGINE CACHE UPDATED SUCCESSFULLY.")

    except Exception as e:
        print(f"CRITICAL ENGINE EXCEPTION ENCOUNTERED: {str(e)}")
    finally:
        IS_UPDATING = False
...
# =========================================================
@router.get("/tbe_all_tracking_data")
def get_tbe_matrix():
    if not LAST_REFRESH and not MASTER_CACHE:
        return {
            "status": "initializing",
            "message": "Rebuilding global TBE KPIs. Please wait...",
            "data": []
        }
    return {
        "status": "success",
...
@router.get("/tbe_individual_report/{mo}")
def get_individual_tbe_flow(mo: str):
    ...
    raise HTTPException(status_code=404, detail=f"No TBE context records registered for search parameter: '{mo}'")
