...
def process_tbe_dashboard_data():
    ...
    try:
        ...
        for _, meta in tbe_aggregation.items():
            ...
            record = {
                "mo": meta["mo"], "final_variant": meta["final_variant"], "qty_req": meta["qty_req"],
                "component_type": meta["component_type"], "sho_qty": meta["ch_qty"], "sho_in": "-",
                "tb_qty": meta["tb_qty"], "tb_out": str(meta["tb_out"]) if meta["tb_out"] else "-",
                "ch_qty": meta["ch_qty"], "ch_in": str(meta["ch_in"]) if meta["ch_in"] else "-",
                "ch_out": str(meta["ch_out"]) if meta["ch_out"] else "-", "status": tracking_status
            }
