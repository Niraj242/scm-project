...
const Afterchannel = () => {
  ...
  // ==========================================
  // MASTER PIPELINE TRACE ENGINE (ASYNC ENDPOINT CALL)
  // ==========================================
  const handleTraceRoute = async () => {
    ...
    let baseQty = 0;
    let baseVariant = "MO Not Indexed in Backend Master Data Sheets";
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.found) {
          ...
          baseVariant = `${data.variant} [${data.source_line} - ${data.sheet_tab}]`;
        }
      }
    } catch (error) {
      console.error("Trace error:", error);
      baseVariant = "Failed to communicate with master data backend.";
    }
      ...
      {activeDept === 'Vibration Dismantling' && (
        <div className="split-layout-container">
          ...
          <div className="forms-grid-split">
            <div className="operation-card container-inbound">
              ...
              <button className="submit-btn btn-in" onClick={() => handleLogTransaction('Vibration', 'IN')}>
                {editingId ? "Update Arrival" : "Log Station Arrival"}
              </button>
            </div>
            ...
            <div className="operation-card container-outbound">
              ...
              <button className="submit-btn btn-out" onClick={() => handleLogTransaction('Vibration', 'OUT')}>
                {editingId ? "Update Release" : "Log Component Release"}
              </button>
            </div>
          </div>
          ...
          <div className="table-wrapper structural-history-space">
            ...
            <table>
              <thead>
                <tr><th>Action</th><th>MO</th><th>Date</th><th>Shift</th><th>Process Metrics</th><th>Quantities</th><th>Actions Hub</th></tr>
              </thead>
              <tbody>
