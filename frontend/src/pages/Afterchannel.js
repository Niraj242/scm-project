...
const Afterchannel = () => {
  ...
  const [scrapDataFetched, setScrapDataFetched] = useState(false);
  ...
  // === LAZY LOAD SCRAP DATA ONLY WHEN TAB IS CLICKED ===
  useEffect(() => {
    const fetchScrapData = async () => {
      if (activeTab === 'scrap' && !scrapDataFetched) {
                  ...
                  {isScrapLoading ? (
                    <tr>
                      <td colSpan="6" style={{textAlign: 'center', padding: '40px'}}>
                         <div style={{fontSize: '16px', color: 'var(--ac-blue-dark)', fontWeight: 'bold'}}>
                           🔄 Connecting to External File & Parsing... This will take a moment.
                         </div>
                      </td>
                    </tr>
                  ) : scrapData.length === 0 ? (
                    <tr><td colSpan="6" style={{textAlign: 'center', padding: '20px'}}>No Scrap Data Found or File is Empty.</td></tr>
                  ) : (
