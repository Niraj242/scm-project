...
const Afterchannel = () => {
  ...
  const [entryMode, setEntryMode] = useState('IN'); // Independent workflow splitter
  ...
  // FIXED: Explicitly stringifies types like 32212 to ensure accurate calculations
  ...
  // Sends structured payloads to prevent backend validation blocks
  const submitForm = async (endpoint, specificPayload) => {
    const basePayload = {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errJson.message || `HTTP Rejection Status ${response.status}`);
      }
  ...
  return (
    <div className="afterchannel-container" style={{padding: '20px', fontFamily: 'sans-serif'}}>
      ...
      {/* --- REWORK DATA LABELS DIRECTLY FROM SPREADSHEETS --- */}
      <datalist id="channels-list">
        {['T1','T2','T3','T4','T5','T6','T7','T8','T9','T10','T11','T12','CH01','CH02','CH03','CH04','CH05','CH06','CH07','CH08','CH09','CH10','CH11','CH12'].map(ch => (
      ...
      <datalist id="cps-items">
        <option value="Seal" /><option value="Shield" /><option value="OM Black" />
        <option value="OM White" /><option value="IM Black" /><option value="IM White" />
      </datalist>

      {/* --- MASTER HEADER REGION --- */}
        ...
        <div style={{marginBottom: '20px', background: '#f8fafc', padding: '15px', borderRadius: '8px', border: '1px solid #e2e8f0'}}>
          <div style={{display: 'flex', gap: '20px'}}>
            <div style={{flex: 1}}>
              ...
              <input type="text" value={moNumber} onChange={(e) => setMoNumber(e.target.value)} onBlur={handleMoBlur} style={{width: '100%', padding: '8px', border: '1px solid #cbd5e1', borderRadius: '4px'}} />
            </div>
            <div style={{flex: 1}}>
              <label style={{display: 'block', fontWeight: '600', marginBottom: '5px'}}>Bearing Variant (Type)</label>
              {availableVariants.length > 0 ? (
                <select value={selectedVariant} onChange={handleVariantChange} style={{width: '100%', padding: '8px', border: '1px solid #cbd5e1', borderRadius: '4px'}}>
                  <option value="">-- Select Variant --</option>
                  {availableVariants.map(v => <option key={v.type} value={v.type}>{v.type}</option>)}
                </select>
              ) : (
                <input type="text" value={selectedVariant} onChange={(e) => setSelectedVariant(e.target.value)} placeholder="Manual Entry Override" style={{width: '100%', padding: '8px', border: '1px solid #cbd5e1', borderRadius: '4px'}} />
              )}
            </div>
            <div style={{flex: 1}}>
              <label style={{display: 'block', fontWeight: '600', marginBottom: '5px'}}>Actual Production Qty</label>
          ...
          {/* Workflow Entry Separator Option */}
      ...
      <div className="ac-content">
        ...
        {activeTab === 'accurate' && (
          <form onSubmit={(e) => {
            ...
            submitForm('accurate', Object.fromEntries(fd));
          }}>
            {entryMode === 'IN' ? (
              <fieldset style={{border: '1px solid #cbd5e1', padding: '15px', borderRadius: '6px'}}><legend style={{fontWeight: 'bold'}}>Accurate - Receiving Log</legend>
                <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '15px'}}>
                  ...
                  <div><label>Shift In</label><select name="shiftIn" style={{width:'100%', padding:'6px'}}><option value=""></option><option value="1">1</option><option value="2">2</option><option value="3">3</option></select></div>
                  <div><label>PC</label><input type="text" name="pc" style={{width:'100%', padding:'6px'}}/></div>
                  <div><label>Material In From</label><select name="materialInFrom" style={{width:'100%', padding:'6px'}}><option value=""></option><option value="Channel">Channel</option><option value="Rework">Rework</option></select></div>
              ...
              <fieldset style={{border: '1px solid #ea580c', padding: '15px', borderRadius: '6px'}}><legend style={{fontWeight: 'bold', color: '#ea580c'}}>Accurate - Dispatch Log</legend>
                <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '15px'}}>
                  <div><label>Next Station</label><select name="nextStation" style={{width:'100%', padding:'6px'}}><option value=""></option><option value="Packaging">Packaging</option><option value="FPS">FPS</option><option value="Rework">Rework</option><option value="Scrap">Scrap</option></select></div>
                  ...
                  <div><label>Shift Out</label><select name="shiftOut" style={{width:'100%', padding:'6px'}}><option value=""></option><option value="1">1</option><option value="2">2</option><option value="3">3</option></select></div>
                </div>
              </fieldset>
            )}
            <button type="submit" style={{marginTop:'15px', padding:'10px 25px', background: entryMode==='IN'?'#2563eb':'#ea580c', color:'#fff', border:'none', borderRadius:'4px', fontWeight:'bold', cursor:'pointer'}}>Save Operational Entry</button>
          </form>
        )}
        ...
        {activeTab === 'cps' && (
          <form onSubmit={(e) => {
            e.preventDefault();
            const fd = new FormData(e.target);
            submitForm('cps', Object.fromEntries(fd));
          }}>
            {entryMode === 'IN' ? (
              <fieldset style={{border: '1px solid #cbd5e1', padding: '15px', borderRadius: '6px'}}><legend style={{fontWeight: 'bold'}}>CPS Assembly - Receiving Log</legend>
                <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '15px'}}>
                  <div><label>Item</label><input list="cps-items" name="item" style={{width:'100%', padding:'6px'}}/></div>
                  <div><label>In Date</label><input type="date" name="inDate" style={{width:'100%', padding:'6px'}}/></div>
                  <div><label>Shift In</label><select name="shiftIn" style={{width:'100%', padding:'6px'}}><option value=""></option><option value="1">1</option><option value="2">2</option><option value="3">3</option></select></div>
                  <div><label>RC No</label><input type="text" name="rcNo" style={{width:'100%', padding:'6px'}}/></div>
                  <div><label>Material In From</label><select name="materialInFrom" style={{width:'100%', padding:'6px'}}><option value=""></option><option value="Channel">Channel</option><option value="Rework">Rework</option><option value="Dismantling">Dismantling</option><option value="Accurate">Accurate</option></select></div>
        ...
        {activeTab === 'rework' && (
          <form onSubmit={(e) => {
            e.preventDefault();
            const fd = new FormData(e.target);
            submitForm('rework', Object.fromEntries(fd));
          }}>
            {entryMode === 'IN' ? (
              <fieldset style={{border: '1px solid #cbd5e1', padding: '15px', borderRadius: '6px'}}><legend style={{fontWeight: 'bold'}}>Rework Station - Receiving Log</legend>
                <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '15px'}}>
                  <div><label>In Date</label><input type="date" name="inDate" style={{width:'100%', padding:'6px'}}/></div>
                  <div><label>Shift</label><select name="shiftIn" style={{width:'100%', padding:'6px'}}><option value=""></option><option value="1">1</option><option value="2">2</option><option value="3">3</option></select></div>
                  <div><label>Channel</label><input list="channels-list" name="channel" style={{width:'100%', padding:'6px'}}/></div>
                  <div><label>Line Segment</label><input type="text" name="lineSegment" style={{width:'100%', padding:'6px'}}/></div>
                  <div><label>Material In From</label><select name="materialInFrom" style={{width:'100%', padding:'6px'}}><option value=""></option><option value="Channel">Channel</option><option value="Accurate">Accurate</option></select></div>
                  <div><label>Qty In</label><input type="number" name="qtyIn" style={{width:'100%', padding:'6px'}}/></div>
              ...
              <fieldset style={{border: '1px solid #ea580c', padding: '15px', borderRadius: '6px'}}><legend style={{fontWeight: 'bold', color: '#ea580c'}}>Rework Station - Dispatch Log</legend>
                <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '15px'}}>
                  <div><label>Next Station</label><select name="nextStation" style={{width:'100%', padding:'6px'}}><option value=""></option><option value="Channel">Channel</option><option value="Accurate">Accurate</option><option value="CPS">CPS</option><option value="Dismantling">Dismantling</option><option value="Scrap">Scrap</option></select></div>
                  <div><label>Qty Sent</label><input type="number" name="qtySent" style={{width:'100%', padding:'6px'}}/></div>
                  <div><label>Out Date</label><input type="date" name="outDate" style={{width:'100%', padding:'6px'}}/></div>
                  <div><label>Shift</label><select name="shiftOut" style={{width:'100%', padding:'6px'}}><option value=""></option><option value="1">1</option><option value="2">2</option><option value="3">3</option></select></div>
        ...
        {activeTab === 'vibration' && (
          <form onSubmit={(e) => {
            e.preventDefault();
            const fd = new FormData(e.target);
            submitForm('vibration', Object.fromEntries(fd));
          }}>
            {entryMode === 'IN' ? (
              <fieldset style={{border: '1px solid #cbd5e1', padding: '15px', borderRadius: '6px'}}><legend style={{fontWeight: 'bold'}}>Vibration Dismantling - Receiving Log</legend>
                <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '15px'}}>
                  ...
                  <div><label>Shift</label><select name="shiftIn" style={{width:'100%', padding:'6px'}}><option value=""></option><option value="1">1</option><option value="2">2</option><option value="3">3</option></select></div>
                  <div><label>Channel</label><input list="channels-list" name="channel" style={{width:'100%', padding:'6px'}}/></div>
                  <div><label>Reason</label><select name="reason" style={{width:'100%', padding:'6px'}}><option value=""></option><option value="D4">D4</option><option value="OD Mark">OD Mark</option></select></div>
                  <div><label>Material In From</label><select name="materialInFrom" style={{width:'100%', padding:'6px'}}><option value=""></option><option value="Channel">Channel</option><option value="Rework">Rework</option></select></div>
                  <div><label>Qty In</label><input type="number" name="qtyIn" style={{width:'100%', padding:'6px'}}/></div>
                  <div><label>Activity</label><select name="activity" style={{width:'100%', padding:'6px'}}><option value=""></option><option value="Ball Remove">Ball Remove</option><option value="Rivet Press">Rivet Press</option></select></div>
                  <div><label>Ring Type</label><select name="ringType" style={{width:'100%', padding:'6px'}}><option value=""></option><option value="IR">IR</option><option value="OR">OR</option></select></div>
                </div>
              </fieldset>
            ) : (
              <fieldset style={{border: '1px solid #ea580c', padding: '15px', borderRadius: '6px'}}><legend style={{fontWeight: 'bold', color: '#ea580c'}}>Vibration Dismantling - Output / Scrap Log</legend>
                <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '15px'}}>
                  ...
                  <div><label>Next Station</label><select name="nextStation" style={{width:'100%', padding:'6px'}}><option value=""></option><option value="Channel">Channel</option><option value="CPS">CPS</option><option value="Scrap">Scrap</option></select></div>
                  <div><label>Qty Sent</label><input type="number" name="qtySent" style={{width:'100%', padding:'6px'}}/></div>
                  <div><label>Out Date</label><input type="date" name="outDate" style={{width:'100%', padding:'6px'}}/></div>
                  <div><label>Shift</label><select name="shiftOut" style={{width:'100%', padding:'6px'}}><option value=""></option><option value="1">1</option><option value="2">2</option><option value="3">3</option></select></div>
                  <div><label>Operator</label><input type="text" name="operator" style={{width:'100%', padding:'6px'}}/></div>
        ...
        {/* ================= SYSTEM CONTROL REFERENCE DATA LAYER ================= */}
        {activeTab === 'summary' && (
      ...
      {/* ================= SUMMARY MODAL SYSTEM FLOW SUMMARY ================= */}
      {selectedMoDetail && (
        <div className="modal-backdrop" style={{position: 'fixed', top:0, left:0, width:'100vw', height:'100vh', background:'rgba(15, 23, 42, 0.6)', display:'flex', justifyContent:'center', alignItems:'center', zIndex: 1000}}>
          <div className="modal-window" style={{background:'#fff', padding:'25px', borderRadius:'8px', width:'95%', maxWidth:'1500px', maxHeight:'85vh', overflowY:'auto'}}>
            ...
            <table style={{width: '100%', borderCollapse: 'collapse', fontSize: '0.88em'}}>
              <thead>
                <tr style={{background: '#f8fafc'}}>
                  ...
                  <th style={{border: '1px solid #cbd5e1', padding: '10px'}}>Accurate IR In</th>
                  <th style={{border: '1px solid #cbd5e1', padding: '10px'}}>Accurate IR Out</th>
                  <th style={{border: '1px solid #cbd5e1', padding: '10px'}}>Accurate OR In</th>
                  <th style={{border: '1px solid #cbd5e1', padding: '10px'}}>Accurate OR Out</th>
              ...
              <tbody>
                {selectedMoDetail.breakdown.map((row, i) => (
                  <tr key={i} style={{borderBottom: '1px solid #e2e8f0', textAlign: 'center'}}>
                    ...
                    <td style={{padding:'10px'}}>{row.irIn || '-'}</td><td style={{padding:'10px'}}>{row.irOut || '-'}</td>
                    <td style={{padding:'10px'}}>{row.orIn || '-'}</td><td style={{padding:'10px'}}>{row.orOut || '-'}</td>
                    ...
                    <td style={{padding:'10px'}}>{row.disIn || '-'}</td><td style={{padding:'10px', color:'#dc2626'}}>{row.scrapSum || '-'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
