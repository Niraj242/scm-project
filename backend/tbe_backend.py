...
# -------------------------------------------------------------------------
# DATE PARSER - EXACT AND STRICT (NO REVERSE ISO SWAPPING)
# -------------------------------------------------------------------------
def parse_date_safe(value, date_format="dd-mm-yyyy"):
    try:
        ...
        # 2. ISO Format (YYYY-MM-DD) natively generated by pandas dtype=str on Date cells
        # We must respect this and NOT swap it
        if re.match(r'^\d{4}-\d{1,2}-\d{1,2}$', val_clean):
            try: return datetime.strptime(val_clean, "%Y-%m-%d").date()
            except ValueError: pass

        # 3. Explicit String Formatting (For Text-formatted cells in Excel)
