            ...
            for s_name, df_b in sheets_box.items():
                ...
                if 'RING' in s_name_up and 'BOX' in s_name_up:
                    ...
                    for r_idx in range(min(20, len(df_fb))):
                        ...
                        if t_c != -1 and (i_cHere is the updated code implementing the requested strict WIP carry-forward logic. 

WIP quantities now accurately deduct from the raw daily demand (eliminating duplicate Heat Treatment scheduling for already-processed quantities) and only the *next pending stage* is stored in the persistence layer. All other logic remains completely untouched.
